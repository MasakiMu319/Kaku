...
> 全局规则在 `~/.claude/CLAUDE.md`，Rust 通用规则在 `~/.claude/rules/rust.md`。本文件只放仓库地图、Working Rules、Verification、Subsystem Guides、Current Risk Areas。发版细节看 `.claude/skills/release/SKILL.md`（通过 `/release` 调用）。
...
- For latest issue/PR sweeps, read live GitHub state first with `gh issue list` and `gh pr list`; refresh once more before final conclusions or public actions.
- Before commenting on or closing an item, confirm its title, state, and author with `gh issue view` or `gh pr view`.
- Do not close issues or PRs on local green alone. For fixes pushed to `main`, wait for the new GitHub Actions run on `main` to complete successfully before posting fixed/closed replies.
...
Tag format `V0.x.x`. `scripts/release.sh` is the source of truth. Title comes from `.github/RELEASE_NOTES.md`. Recovery is idempotent when the tag already points at the intended commit. Full runbook in `.claude/skills/release/SKILL.md`.
